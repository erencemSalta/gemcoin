...
def test_trascation_update_exceeds_balance():
    sender_wallet = Wallet()
...
def test_transaction_update():
    ...
    assert transaction.output[sender_wallet.address] == sender_wallet.balance - first_amount - to_first_again_amount - next_amount
    assert Wallet.verify(
        transaction.input['public_key'],
        transaction.output,
        transaction.input['signature']
